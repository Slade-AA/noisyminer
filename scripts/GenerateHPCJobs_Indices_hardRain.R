...
batchJobTemplate = "scripts/batchJobScript_calculateIndices_hardRain.sh"
...
numberFullJobs = floor(nrow(ListOfRecordings)/(100*15))
lengthLastJob = floor(nrow(ListOfRecordings)%%(100*15)/15) #this may leave off a few recordings at the end if they are not divisible by 5!
...
dir.create(path = "HPCJobScripts/hardRain", recursive = TRUE, showWarnings = FALSE)
...
for (row in 1:nrow(batchJobArrayIndices)) {
  ...
  #JobName
  scriptFile[4]=paste0("#PBS -N GenerateIndicesHardRain", row)
  #ArrayIndices
  scriptFile[8]=paste0("#PBS -J ", batchJobArrayIndices$startIndex[row], "-", batchJobArrayIndices$endIndex[row])
  
  currentFile = paste0("HPCJobScripts/hardRain/JobScript", row, ".sh")
...
JobSubmission[1] <- paste0("FIRST=$(qsub \"NoisyMiner/HPCJobScripts/hardRain/JobScript1.sh\")")
JobSubmission[2] <- "echo $FIRST"

if (nrow(batchJobArrayIndices) > 1) {
  for (job in rownames(batchJobArrayIndices)[-1]) {
    JobSubmission[(2*as.numeric(job))-1] <- paste0(gsub(" |-", "", toupper(english::ordinal(as.numeric(job)))), "=$(qsub -W depend=afterany:$", gsub(" |-", "", toupper(english::ordinal(as.numeric(job)-1))), " \"NoisyMiner/HPCJobScripts/hardRain/JobScript", job, ".sh\")")
    JobSubmission[(2*as.numeric(job))] <- paste0("echo $", gsub(" |-", "", toupper(english::ordinal(as.numeric(job)))))
  }
}

writeLines(JobSubmission, paste0("HPCJobScripts/hardRain/_JobSubmission.txt"))