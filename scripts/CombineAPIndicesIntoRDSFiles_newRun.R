...
dirs <- list.dirs("G:/_newRun")
dirs <- dirs[grepl("Towsey.Acoustic", dirs)]

dirsAll <- dirs

dirs <- dirsAll[10001:54368] #1:10000 DONE
...
for (dir in dirs) {
  ...
  if (length(csvFiles) == 0) {
    missingIndices <- c(missingIndices, dir)
    next
  }
  ...
  indices[["info"]] <- list(Site = gsub("[A-Z]:/_newRun/([A-Z0-9]{2,4})/.*", "\\1", dir),
                            Date = as.Date(gsub("[A-Z]:/_newRun/([A-Z0-9]{2,4})/([0-9]{8})_.*", "\\2", dir), "%Y%m%d"), #this doesn't work on a handful of files with different naming structure (possible fix is to add '.*' before second group capture - need to test)
                            Time = case_when(
  ...
  dir.create(path = paste0(gsub("([A-Z]:/).*", "\\1",dir), "_newRun/_combinedIndices/Indices_AP/", indices[["info"]]$Site),
             recursive = T, showWarnings = F)
  dir.create(path = paste0(gsub("([A-Z]:/).*", "\\1",dir), "_newRun/_combinedIndices/Indices_R/", indices[["info"]]$Site),
             recursive = T, showWarnings = F)
  
  #save RDS object with unique filename
  saveRDS(object = indices,
          file = paste0(gsub("([A-Z]:/).*", "\\1", dir), "_newRun/_combinedIndices/Indices_AP/", indices[["info"]]$Site, "/", indices[["info"]]$Site, "_", indices[["info"]]$Date, "_", gsub("__Towsey.Acoustic.*", "", basename(csvFiles[1])), ".RDS"))
  
  #copy R indices over to same folder structure
  file.copy(from = list.files(gsub("Towsey.Acoustic", "indices_R", dir), full.names = T),
            to = paste0(gsub("([A-Z]:/).*", "\\1", dir), "_newRun/_combinedIndices/Indices_R/", indices[["info"]]$Site, "/", indices[["info"]]$Site, "_", indices[["info"]]$Date, "_", gsub("__Towsey.Acoustic.*", "", basename(csvFiles[1])), ".RDS"))
}